    ...
    <div className={styles.brand_item_wrapper}>
      ...
      <div>
        ...
        <p className={styles.price_info}>
          <span className={styles.original_price}>{numberWithCommas(10000)}</span>원{' '}
          <span className={styles.current_price}>{numberWithCommas(9000)}원</span>
          <span className={styles.sale_percent}> 15%</span>
        </p>
        <LikePin defaultColor="skeleton" position={{ top: '10%', right: '3%' }} />
      </div>
    </div>
  );
};
